#!/usr/bin/env python3
"""Test crypto queries with MCP server integration"""
...
async def test_crypto_queries():
    """Test crypto-related queries with MCP server"""
    
    print("🔍 Testing Crypto Queries with MCP Server")
    print("=" * 50)
            ...
            try:
                ...
                if result.servers:
                    for j, server_result in enumerate(result.servers[:3], 1):
                        ...
                        print(f"     Description: {server_result.description[:100]}...")
                else:
...
if __name__ == "__main__":
    asyncio.run(test_crypto_queries()) 